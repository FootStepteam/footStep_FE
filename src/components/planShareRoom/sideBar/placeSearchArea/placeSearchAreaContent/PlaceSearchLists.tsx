        ...
        {isExist ? (
          <ul id="placesList">
            {placeLists.map((place: IKakaoPlaceSearchResult, index) => (
              <li
                key={place.id}
                className={`flex py-4 hover:bg-gray-005 border-b border-b-gray-003 cursor-pointer ${index === 0 && 'border-t border-t-gray-003'}`}
              >
                <div className="ml-4 w-[19em]">
                  <div className="mb-1 flex items-center">
                    <p className="font-medium text-xl">{place.place_name}</p>
                    <p className="text-gray-002 text-sm ml-2">{place.category_group_name}</p>
                  </div>
